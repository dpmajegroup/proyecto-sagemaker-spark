...
FILE_KEY_TOMORROW = "pedido_sugerido/data-v1/cam/ventas_PA_tomorrow.csv"
...
def generar_pedido_estrategico():
    ...
    print("Generando Pedido Estrategico (Panama)...")
...
def excluir_recurrente_y_sugerido(df_final):
    ...
    # Leer Recurrente
    try:
        pr_pa = wr.s3.read_csv(
            f"s3://{S3_BUCKET_BACKUP}/Pedido_Recurrente/Panama/Output/recu_base_pedidos_{FECHA_REC}.csv",
            boto3_session=my_session
        )[["Compania", "Cliente", "Producto"]]
    except Exception as e:
        print(f"No se pudo leer Recurrente: {e}")
        pr_pa = pd.DataFrame(columns=["Compania", "Cliente", "Producto"])

    # Leer Sugerido
    try:
        ps_pa = wr.s3.read_csv(
            f"s3://{S3_BUCKET_BACKUP}/PS_Panama/Output/PS_todo_panama/D_base_pedidos_{FECHA_REC}.csv",
            boto3_session=my_session
        )[["Compania", "Cliente", "Producto"]]
    except Exception as e:
        print(f"No se pudo leer Sugerido: {e}")
        ps_pa = pd.DataFrame(columns=["Compania", "Cliente", "Producto"])

    quitar_temp = pd.concat([pr_pa, ps_pa], ignore_index=True).drop_duplicates()
...
def leer_estrategico_externo():
    ...
    key = "aje/analiticaAvanzada/pa/pedido_estrategico/Pedido Estrategico.csv"
    local_path = "/opt/ml/processing/Pedido_Estrategico_PA.csv"
...
def exportar_y_concatenar(df_estrategico):
    ...
    s3_path_backup = f"s3://{S3_BUCKET_BACKUP}/Pedido_Estrategico/Panama/Output/estr_base_pedidos_{FECHA_REC}.csv"
...
def main():
    print("--- INICIANDO PEDIDO ESTRATEGICO (Panama) ---")
