...
# Ruta del consolidado actualizado (ya incluye países tarde gracias a RPT_5)
...
def leer_csv_desde_zip(zip_key):
    ...
    print(f"  Errores cargados (total): {df.shape[0]} filas")
    return df


def filtrar_paises_tarde(df_errores):
    """Filtra solo los errores de países tarde (Nicaragua, Colombia)."""
    df_errores["Pais"] = df_errores["Pais"].astype(str).str.strip()
    df_filtrado = df_errores[df_errores["Pais"].isin(PAISES_TARDE)].copy().reset_index(drop=True)
    print(f"  Errores filtrados (países tarde): {df_filtrado.shape[0]} filas")
    return df_filtrado
...
def cargar_consolidado_subido():
    ...
    try:
        ...
        # Filtrar solo países tarde
        df["Pais"] = df["Pais"].astype(str).str.strip()
        df = df[df["Pais"].isin(PAISES_TARDE)].reset_index(drop=True)
        print(f"  Consolidado (países tarde): {df.shape[0]} filas")
        return df
...
def construir_html(tabla1, tabla2, tabla3, tabla_otros, zip_key):
    ...
    <body>
    <h2>⚠️ Validación de Errores - Países Tarde ({paises_str})</h2>
    <p>Fecha de recomendaciones: <b>{fecha_tomorrow}</b></p>
    <p>Archivo: <code>{zip_key}</code></p>

    <div class="info">
        <b>Nota:</b> Esta validación complementa la validación principal.
        Incluye solo errores de países con input externo ({paises_str}).
    </div>
...
def enviar_correo(html_body):
    """Envía el correo con el reporte de errores tarde."""
    print("Enviando correo de validación tarde...")
    msg = MIMEMultipart()
    msg["From"] = REMITENTE
...
def main():
    print("--- INICIANDO VALIDACIÓN DE ERRORES TARDE (PAÍSES CON INPUT EXTERNO) ---")
    ...
    # 3. Filtrar solo países tarde
    df_errores = filtrar_paises_tarde(df_errores)
    if df_errores.empty:
        print("No hay errores para países tarde. Finalizando.")
        return

    # 4. Clasificar errores y deduplicar
    df_errores = clasificar_errores(df_errores)

    # 5. Cargar consolidado de pedidos subidos (solo países tarde)
    df_subido = cargar_consolidado_subido()

    # 6. Generar tablas
    tabla1, tabla2, tabla3, tabla_otros = generar_tablas(df_errores, df_subido)

    # 7. Construir HTML y enviar correo
    html_body = construir_html(tabla1, tabla2, tabla3, tabla_otros, zip_key)
    enviar_correo(html_body)

    print("--- VALIDACIÓN TARDE FINALIZADA ---")
